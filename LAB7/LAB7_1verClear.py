...
class Account:
    ...
    def __iter__(self):
        for transaction in self.__transaction:
            if transaction.transaction_type == "out_tranfer":
                yield f"Transaction Type: {transaction.transaction_type}, Amount : {transaction.amount}, Remaining Balance : {transaction.total}, Target Account : {transaction.target_account.user.name}"
            else:
                yield f"Transation Type: {transaction.transaction_type}, Amount : {transaction.amount}, Remaining Balance : {transaction.total}"
...
class Bank:
    ...
    def add_seller(self, sellerer : Seller):
...
hermione_account = scb.search_account_from_account_no('0987654321')
print("Test Case #7")
print("Hermione's Account Transactions : ")
